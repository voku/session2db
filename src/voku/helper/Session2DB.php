...
class Session2DB implements \SessionHandlerInterface
{
     ...
     *
     * @param string          $security_code      [Optional] The value of this argument is appended to the string
     *                                            created by concatenating the user's User Agent (browser) string (or
     *                                            an empty string if "lock_to_user_agent" is FALSE) and to the user's
     *                                            IP address (or an empty string if "lock_to_ip" is FALSE), before
     *                                            creating an SHA1 hash out of it and storing it in the database.
     *
     ...
     *                                          hijacking.
     *
     *                                          <samp>To prevent session hijacking, make sure you choose a string
     *                                          around
     *                                          12 characters long containing upper- and lowercase letters, as well as
     *                                          digits. To simplify the process, use
     *                                          {@link *                                          https://www.random.org/passwords/?num=1&len=12&format=html&rnd=new this} link to generate such a random string.</samp>
     * @param  int            $session_lifetime   [Optional] The number of seconds after which a session will be
     *                                            considered as <i>expired</i>.
     *
     *                                          Expired sessions are cleaned up from the database whenever the
     *                                          <i>garbage collection routine</i> is run. The probability of the
     *                                          <i>garbage collection routine</i> to be executed is given by the values
     *                                          of
     *                                          <i>$gc_probability</i> and <i>$gc_divisor</i>. See below.
     *
     *                                          Default is the value of <i>session.gc_maxlifetime</i> as set in in
     *                                          php.ini. Read more at
     *                                          {@link *                                          http://www.php.net/manual/en/session.configuration.php}
     *
     *                                          To clear any confusions that may arise: in reality,
     *                                          <i>session.gc_maxlifetime</i> does not represent a session's lifetime
     *                                          but the number of seconds after which a session is seen as
     *                                          <i>garbage</i> and is deleted by the <i>garbage collection routine</i>.
     ...
     *
     *                                          <i>The user agent check only adds minor security, since an attacker
     *                                          that
     *                                          hijacks the session cookie will most likely have the same user
     *                                          agent.</i>
     *
     ...
     *
     *                                          Default is FALSE.
     * @param bool            $lock_to_ip         [Optional]    Whether to restrict the session to the same IP as when
     *                                            the
     ...
     *                                            the probability that the <i>garbage collection routine</i> is
     *                                            started.
     *
     ...
     *                                          means
     *                                          that there is a 1% chance the the <i>garbage collection routine</i>
     *                                          will
     *                                          be called on each request.
     *
     *                                          Default is the value of <i>session.gc_probability</i> as set in
     *                                          php.ini.
     *                                          Read more at
     *                                          {@link *                                          http://www.php.net/manual/en/session.configuration.php}
     *
     ...
     *
     *                                          <i>This must be lower than the maximum execution time of the
     *                                          script!</i>
     *
     ...
     *                                            starting the session, you can skip the session-start and do it
     *                                            manually via "Session2DB->start()"
     */
